#!/usr/bin/env python

# This script converts between Roll-Pitch-Yaw angles and Quaternions

import rospy
from geometry_msgs.msg import Twist
from nav_msgs.msg import Odometry
import math
import tf

print('-----------------------------------------')
print('Roll-Pitch-Yaw Conversion to Quaternions')
print('-----------------------------------------')

# Conversion 1
# Define some random angles roll, pitch and yaw in radians
roll = math.radians(30)
pitch = math.radians(42)
yaw = math.radians(58)

print("roll = {}, pitch = {}, yaw = {}".format(math.degrees(roll),math.degrees(pitch),math.degrees(yaw)))

# Convert roll-pitch-yaw angles -> quaternion
quaternion = tf.transformations.quaternion_from_euler(roll, pitch, yaw)
print('Conversion 1')
for i in range(4):
	print(quaternion[i])

#quaternion[0] = -3.88256895463e-06
#quaternion[1] = 0.0015896463485
#quaternion[2] = 0.001397167245
#quaternion[3] = 0.999997760464

# Conversion 2
# Perform the opposite converion
rpy = tf.transformations.euler_from_quaternion(quaternion)
roll_from_quaternion = rpy[0]
pitch_from_quaternion = rpy[1]
yaw_from_quaternion = rpy[2]

# Convert q -> roll-pitch-yaw using euler_from_quaternion
print('Conversion 2')
print('roll = {}, pitch = {}, yaw = {}'.format(math.degrees(roll_from_quaternion),math.degrees(pitch_from_quaternion),math.degrees(yaw_from_quaternion)))

# This gives the same initial roll-picth-roll values')

# Conversion 3
# Define a new quaternion manually as a list
q = (-3.88256895463e-06, 0.0015896463485, 0.001397167245, 0.0)

# Perform conversion q -> euler
rpy = tf.transformations.euler_from_quaternion(q)
roll_from_quaternion = rpy[0]
pitch_from_quaternion = rpy[1]
yaw_from_quaternion = rpy[2]
print('Conversion 3')
print('roll = {}, pitch = {}, yaw = {}'.format(math.degrees(roll_from_quaternion),math.degrees(pitch_from_quaternion),math.degrees(yaw_from_quaternion)))

# you can change these values in the file to make other converions